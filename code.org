...
** TODO specify queue with command line argument

Some alternative options or enhancements

+ Might set this using the python interface too, since all simulations are prepared there anyways.

+ automatically call a load balancer on cpt hosts
  + can be very easy, just look in bjobs and pick the queue with the least jobs


** TODO add possibility to toogle or set build options for Fargo3d, parser

+ require the option to be already present
+ just comment/uncomment options
* ParamSet
...
Install the =sheep= to your user directory with

#+BEGIN_SRC sh
python3 setup.py install --user
#+END_SRC

which runs the setup script
...
setup(  name="sheep"
        ,version="0.1"
        ,description="Simulation HElper for Exploring Parameter space."
        ,author="Thomas Rometsch"
		,packages=['sheep']
        )
...
The package dir needs to include an file called =__init__.py= for python to know that there are modules inside the directory.
To conveniently access the =Sheep= class =sheep.Sheep=, the =sheep.py= file is also imported in the init file.

Note the dot in front of sheep. With out it, the import ends in an infinite recursion loop!

#+BEGIN_SRC python :tangle sheep/__init__.py
from .sheep import *
#+END_SRC
